...
**Lint config excludes `docs`.** `ruff format` reformats Python code blocks
inside markdown, so `docs/ARCHITECTURE.md` §3.3 gets rewritten and a repo-wide
`--check` fails on a file that is documentation, not source. `pyproject.toml`
carries `extend-exclude = ["docs"]`, which Phase 2 confirmed is sufficient —
`ruff format --check .` passes with `.opencode/` and `AGENTS.md` in scope,
because ruff does not read markdown by default. Do not widen it without a
demonstrated failure; an unjustified exclusion is its own review finding.
This rule is stated in `AGENTS.md` under "Project-wide tooling rules" — cite
that, not this file.
...
When you report a rule — in a `DEFERRED` item, an assumption, or a justification
— name where it actually comes from. A rule you read in **this file** is your own
standing instruction; only `AGENTS.md`, `docs/ARCHITECTURE.md` and
`docs/INSTRUCTIONS.md` are citable project documents. In Phase 2 a subagent
deferred an item claiming "AGENTS.md prescribes `extend-exclude = [...]`" for a
rule that lived only here; the orchestrator grepped, found nothing, and the phase
spent a review cycle and a GitHub issue adjudicating a false premise. If you
cannot point to the file and the line, say "my standing instructions" instead.
